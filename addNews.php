...
$news = $bdd->query('SELECT * FROM iss_news ORDER BY date_publication DESC');
$mode_edition = 0;

if(isset($_GET['edit']) AND !empty($_GET['edit'])) {
  $mode_edition = 1;
  $edit_id = htmlspecialchars($_GET['edit']);
  $edit_news = $bdd->prepare('SELECT * FROM iss_news WHERE id = ?');
      ...
      if(isset($_POST['title'],$_POST['description'])){
        if(!empty($_POST['title']) AND !empty($_POST['description'])){
          ...
          $target = "./img/news/".basename($image);// cet function permet d'indiquer le chemin ou les images seront downloader
            ...
            if($mode_edition == 0) {
              $insertmbr = $bdd->prepare("INSERT INTO iss_news (title,description,picture,date_publication) VALUES (?, ?, ?, NOW())");
            ...
            elseif($mode_edition == 1) {
              $update = $bdd->prepare('UPDATE iss_news SET title = ?, description = ?, picture = ?, date_publication = NOW() WHERE id = ?');
...
if(isset($_GET['id']) AND !empty($_GET['id'])) {
  ...
  $suppr = $bdd->prepare('DELETE FROM iss_news WHERE id = ?');
        ...
        <div class="row">
          <div class="col-lg-4 col-sm-4">
            <table class="table table-striped table-bordered table-dark">
              <caption class="panel-heading">ISS | Les news</caption>
              <thead>
                <tr>
