...
if(isset($_GET['edit']) AND !empty($_GET['edit'])) {
  ...
  if($edit_user->rowCount() == 1) {// permet de regarder si le compte existe ou pas
    $edit_user = $edit_user->fetch();
      ...
      if(isset($_POST['name'], $_POST['model'], $_POST['description'], $_POST['price'], $_POST['mark'])){
        if(!empty($_POST['name']) AND !empty($_POST['model']) AND !empty($_POST['description']) AND !empty($_POST['price']) AND !empty($_POST['mark'])){
          ...
          $mark= htmlspecialchars($_POST['mark']);
            ...
            if($mode_edition == 0) {
              $insertmbr = $bdd->prepare("INSERT INTO iss_producs (name,model,mark,description,price,picture,date) VALUES (?, ?, ?, ?, ?, ?, NOW())");
              $insertmbr->execute(array($name,$model,$mark,$description,$price,$image));
              if (move_uploaded_file($_FILES['picture']['tmp_name'], $target)) {// cet function permet de sauvegarder l'image uploader vers le chemin de destination 
                ...
                header('Location: http://iss-tahiti/addProducs.php');
            }

            elseif($mode_edition == 1) {
              $update = $bdd->prepare('UPDATE iss_producs SET name = ?, model = ?, description = ?, price = ?, date=NOW() WHERE id = ?');
              $update->execute(array($name,$model,$description,$price,$edit_id));
...
if(isset($_GET['id']) AND !empty($_GET['id'])) {
  ...
  $suppr = $bdd->prepare('DELETE FROM iss_producs WHERE id = ?');
    ...
    <nav class="navbar fixed-top navbar-expand-lg navbar-light" style="background-color: #52ec90;" "fixed-top">
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
          ...
          <li class="nav-item">
              <a class="nav-link" href="addProducs.php">Ajouter des produits</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="addNews.php">Ajouter des news</a>
            </li>
          </ul>
        </div>
      </div> 
          ...
          <div class="row align-items-center">
            <div class="col-lg-4 col-sm-4">
              <table class="table table-striped table-bordered table-dark">
                <caption class="panel-heading">ISS | Les produits</caption>
                <thead>
                  <tr>
            ...
            <h2 id="producs">Nouvel offre</h2>
              <form method="POST" action="addProducs.php" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  ...
                  $producModels = array("Choose...","Imprimante","Serveur","Oridnateur portable","Ordinateur de bureau","écrans");
                  ?>
                  <select name="model">
                      <?php
                ...
                <div class="form-group">
                  ...
                  $producsMark = array("Choose...","DELL","Terra","Lenovo","Cisco","Lexmark","IBM");
                  ?>
                  <select name="mark">
                      <?php
                          foreach($producsMark as $marque){
                          ?>
